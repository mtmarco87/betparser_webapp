...
export class MatchFilter {
  ...
  FromDate: moment.Moment; // null => everything / moment() => Today
  All: string[];
  Name: string[];
  Team1: string[];
  Team2: string[];
  ...
  applyFilter(matchGroups: MatchGroup[]): MatchGroup[] {
    ...
    matchGroups.forEach((matchGroup) => {
      ...
      matchGroup.children.forEach((match) => {
        ...
        if (
          this.FromDate &&
          !BetsUtils.parseMatchDate(
            match.StartDate,
            BetsUtils.AppDateFormat
          ).isSameOrAfter(this.FromDate, "day")
        ) {
          return;
        }
              ...
              valid =
                ...
                (match.Name.toLowerCase().includes(searchTxt) ||
                  ...
                  match.children.some((bm) => {
                    return (
                      bm.Bookmaker.toLowerCase().includes(searchTxt) &&
                      bm.hasQuotesAvailable()
                    );
                  }));
            });
          } else if (this.Name) {
